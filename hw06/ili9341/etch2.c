...
int main()
{
    ...
    char *fbp = 0;
    int x = 1, y = 0;       // Make it so the it runs before the encoder is moved
    int xold = 0, yold = 0;
    long int location = 0;
	...
	while(rc_get_state() != EXITING) {
        ...
        x = (rc_get_encoder_pos(1)/8 + vinfo.xres) % vinfo.xres;
        y = (rc_get_encoder_pos(3)/8 + vinfo.yres) % vinfo.yres;
        // printf("xpos: %d, xres: %d\n", rc_get_encoder_pos(1), vinfo.xres);
        
        if((x != xold) || (y != yold)) {
            printf("Updating location to %d, %d\n", x, y);
            // Set old location to green <- changed to purple
             int r = 17;     // 5 bits
            int g = 17;      // 6 bits
            int b = 17;      // 5 bits
